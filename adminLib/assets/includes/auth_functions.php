...
function check_verified()
{
    ...
    if (isset($_SESSION['auth'])) {
        ...
        if ($_SESSION['auth'] == 'verified') {
            ...
            echo "<script> window.location.href = '../_admin_verify_admin_/'; </script>";
...
function check_remember_me()
{
    ...
    if (empty($_SESSION['auth']) && !empty($_COOKIE['rememberme'])) {
        ...
        $sql = "SELECT * FROM admin_auth_tokens WHERE auth_type='remember_me' AND selector=? AND expires_at >= NOW() LIMIT 1;";
        $stmt = mysqli_stmt_init($conn);
