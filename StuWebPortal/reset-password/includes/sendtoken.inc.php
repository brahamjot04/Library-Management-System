...
if (isset($_POST['resentsend'])) {
    ...
    */
    ...
    $url = "localhost/StuWebPortal/reset-password/?selector=" . $selector . "&validator=" . bin2hex($token);
    // $url = "https://library.gndpoly.org/StuWebPortal/reset-password/?selector=" . $selector . "&validator=" . bin2hex($token);
    $expires = 'DATE_ADD(NOW(), INTERVAL 1 HOUR)';

    $email = $_POST['email'];

    $sql = "SELECT id FROM stu_users WHERE email=?;";
